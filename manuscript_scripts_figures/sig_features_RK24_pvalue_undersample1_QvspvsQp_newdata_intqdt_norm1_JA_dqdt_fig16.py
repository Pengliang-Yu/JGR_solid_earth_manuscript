...
def main():
    ...
    df_FM=pd.read_csv(r'Z:\MLproject\whakaari-master\features\test_features_RK24_all_int_qdt_norm0909.csv',index_col=0, parse_dates=[0,], infer_datetime_format=True)
    ...
    for i in FM_list:
        ...
        for ii in range(try_time):
            ...
            fts_Q=[(ind,s) for ind,s in enumerate(fts) if ('Flow_rate' in s and 'diff_Flow_rate' not in s)]
            pvs_Q=pvs[[i[0] for i in fts_Q][:]]
            pvs_Q= [x for x in pvs_Q if ~np.isnan(x)]
            
            fts_dQ=[(ind,s) for ind,s in enumerate(fts) if ('diff_Flow_rate' in s)]
            pvs_dQ=pvs[[i[0] for i in fts_dQ][:]]
            pvs_dQ= [x for x in pvs_dQ if ~np.isnan(x)]

            
            fts_whp=[(ind,s) for ind,s in enumerate(fts) if ('WHP' in s and 'diff_WHP' not in s)]
            pvs_whp=pvs[[i[0] for i in fts_whp][:]]
            pvs_whp=[x for x in pvs_whp if ~np.isnan(x)]
            

            fts_QP=[(ind,s) for ind,s in enumerate(fts) if ('Q*P' in s and 'diff_Q*P' not in s)]
            ...
            len_pvs_Q=len(pvs_Q)
            len_pvs_dQ=len(pvs_dQ)
            len_pvs_whp=len(pvs_whp)
            len_pvs_QP=len(pvs_QP)
            len_pvs_int_qdt=len(pvs_int_qdt)
            minlen_pvs=min(len_pvs_Q,len_pvs_dQ,len_pvs_whp,len_pvs_QP,len_pvs_int_qdt)##maka a same len of pvalue_Q, whp, Qp

            pvalue_Q_ave_one_list.append(np.sum(np.log10(pvs_Q[:minlen_pvs])))
            pvalue_dQ_ave_one_list.append(np.sum(np.log10(pvs_dQ[:minlen_pvs])))
        ...
        pvalue_dQ_list.append(np.mean(np.array(pvalue_dQ_ave_one_list)))
    ...
    pvalue_dQ_list_diffPercentile=np.array(pvalue_dQ_list).reshape((len(percentile_list),len(DD)))
    ...
    for i in range(len(percentile_list)):
        ...
        ax.plot(DD,pvalue_dQ_list_diffPercentile[i,:],c='c',marker='s',ms=5,label=r'$\.q$')
        ax.plot(DD,pvalue_whp_list_diffPercentile[i,:],c='k',marker='s',ms=5,label='P')
        ax.plot(DD,pvalue_QP_list_diffPercentile[i,:],c='b',marker='s',ms=5,label=r'E')
        ax.plot(DD,pvalue_int_qdt_list_diffPercentile[i,:],c='g',marker='s',ms=5,label=r'Q')
        ...
        ax.set_title('Well #3, $\lambda_{{th}}$ = {} percentile $\lambda$'.format(percentile_list[i]),fontsize=12)
    ax.legend(loc='best',fontsize=12)
    ax.text(0.95, 0.95, 'a',fontsize=14, horizontalalignment='center', verticalalignment='center', transform=ax.transAxes)
    plt.setp(ax.get_xticklabels(), Fontsize=10)
    plt.setp(ax.get_yticklabels(), Fontsize=10)
    plt.tight_layout()
    plt.savefig(r'Z:\MLproject\whakaari-master\RK24\RK24_pvalues_percentile_balanced2_newQP_norm1_JA1223_1026.png',dpi=500)
    plt.show()
    '''  
