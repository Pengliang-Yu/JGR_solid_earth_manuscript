...
def main():
    ...
    well_data_path=[r'Z:\MLproject\whakaari-master\features\test_features_RK20_part2_1024_norm.csv',
        r'Z:\MLproject\whakaari-master\features\test_features_RK23_part2_1024_norm.csv',
        r'Z:\MLproject\whakaari-master\features\test_features_RK24_part2_1024_norm.csv']
    # well_name=['RK20 well','RK23 well','Rk24 well','Q_total']
    well_name=['RK20 well','RK23 well','RK24 well']
    # well_name=['Well #1-Part 2','Well #2-Part 2','Well #3-Part 2']
    ...
    for i in range(len(percentile_list)):
        for j in range(len(well_data_path)):
            ...
            ax[i].set_title('Post Sep. 2013 period, $\lambda_{{th}}$ = {} percentile $\lambda$'.format(percentile_list[i]),fontsize=12)
            ax[i].legend(loc='best',fontsize=12)
            ax[i].set_ylim([-3500,-400])
            plt.setp(ax[i].get_xticklabels(), Fontsize=10)
            plt.setp(ax[i].get_yticklabels(), Fontsize=10)
    ax[0].text(0.05, 0.05, 'c',fontsize=14, horizontalalignment='center', verticalalignment='center', transform=ax[0].transAxes)
    ax[1].text(0.95, 0.05, 'd',fontsize=14, horizontalalignment='center', verticalalignment='center', transform=ax[1].transAxes)
    '''
    ...
    # ax.legend(loc='best')
    plt.tight_layout()
    plt.savefig(r'Z:\MLproject\whakaari-master\RK20\Threewells_pva_percentile_balanced_reduced_newda_2subp1_part2_norm_1026.png',dpi=500)
    plt.show()
