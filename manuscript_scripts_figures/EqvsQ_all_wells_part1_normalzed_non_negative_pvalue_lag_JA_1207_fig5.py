...
def main():
    ...
    # out_file .to_csv(r'Z:\MLproject\whakaari-master\data\AIC_normalized_part1.dat', index=True)
    ...
    ax.plot(Y_data.index,Y_data,'k-',label='Observed pre-Jul. 2013 data')
    ...
    ax.plot(Y_data.index,Y_model,'r-',label='Pre-Jul. 2013 regression model')
    ...
    ax.text(0.95, 0.05, 'a',fontsize=14, horizontalalignment='center', verticalalignment='center', transform=ax.transAxes)
    plt.tight_layout()
    plt.savefig(r'Z:\MLproject\whakaari-master\regression_fitting\linear_fitting_Qall_part1_norm_NNLS_{}wLag_JA_1026.png'.format(day_num),dpi=500)
    plt.show()  
