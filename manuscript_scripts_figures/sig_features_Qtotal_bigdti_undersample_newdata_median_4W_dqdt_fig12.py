...
Dtj_df=Dti+4
Nbins=60
def one_well_data(FM_path,column_keyword,feature_i_th):
    ...
    for j in range(len(df_FM_list)):
        ...
        fts_WHP_1=fts_WHP[feature_i_th]
        pvs_WHP_1=pvs[fts_WHP_1[:][0]]

        FM_WHP=df_FM_us[fts_WHP_1[:][1]]
...
def main():
    f,axq = plt.subplots(2,2,figsize=(8,8))
    ...
    # FM_data_path=[r'H:\UOAPC_Zdrive\MLproject\whakaari-master\features\test_features_RK24_0726.csv',
    #     r'H:\UOAPC_Zdrive\MLproject\whakaari-master\features\test_features_RK20_0622.csv',
    #     r'H:\UOAPC_Zdrive\MLproject\whakaari-master\features\test_features_RK23_0726.csv',
    #     r'H:\UOAPC_Zdrive\MLproject\whakaari-master\features\test_features_RK24_0726.csv']
    column_keyword=['diff_Q_total(t/h)','diff_Flow_rate(t/h)','diff_Flow_rate(t/h)','diff_Flow_rate(t/h)']
    # column_keyword=['diff_Flow_rate(t/h)','diff_Flow_rate(t/h)','diff_Flow_rate(t/h)','diff_Flow_rate(t/h)']
    ...
    # well_keyword=['Well #3','Well #1','Well #2','Well #3']
    # feature_i_th_list=[1,0,25,2]
    feature_i_th_list=[0,0,1,0]
    number_oder=['(a)','(b)','(c)','(d)']
    for i in range(len(FM_data_path)):
        FM_sig_Q_LB_1,FM_sig_Q_LB_0,pvs_WHP_1,fts_WHP_1=one_well_data(FM_data_path[i],column_keyword[i],feature_i_th_list[i])
        ...
        axq[i].set_xlabel('\n'.join(wrap('{} {}: {}; p_value ={:.2e}'.format(number_oder[i],well_keyword[i],fts_WHP_1[1],pvs_WHP_1),30)),fontsize=12)
    ...
    # axq[2].set_ylim([0,50])
    axq[3].set_xlim([-5,50])
    plt.suptitle('$\lambda_{{th}}$ = {} percentile $\lambda$, $\Delta t$={} days, $\Delta t_j$={} days'.format(percentile_list[0]+20,Dti,Dtj_df),fontsize=14)
    plt.tight_layout()
    plt.savefig(r'H:\UOAPC_Zdrive\MLproject\whakaari-master\Q_total\T3wells_Q_Lambda_th_{}_dti_{}d_dtj_{}d08021_4W_dqdt1026.png'.format(percentile_list[0],Dti,Dtj_df),dpi=500)
    plt.show()
