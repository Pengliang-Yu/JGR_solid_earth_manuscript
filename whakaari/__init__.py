...
class TremorData(object):
    ...
    def update(self, ti=None, tf=None):
        ...
        self.df = self.df.resample('1H').interpolate('linear')
...
class ForecastModel(object):
    ...
    def __init__(self, window, overlap, look_forward, ti=None, tf=None, data_streams=['rsam','mf','hf','dsar'], root=None):
        ...
        self.dt = timedelta(seconds=3600)##the diff of data time
        self.dto = (1.-self.overlap)*self.dtw
        self.iw = int(self.window*24)## in hours         
        self.io = int(self.overlap*self.iw)      
        if self.io == self.iw: self.io -= 1

        self.window = self.iw*1./(24)##in days
        self.dtw = timedelta(days=self.window)
    ...
    def _extract_features(self, ti, tf):
        ...
        Nw = int(np.floor(((tf-ti)/self.dt)/(self.iw-self.io)))+1 ##plus +1
        ...
        # ys = pd.DataFrame(self._get_label(fm.index.values), columns=['label'], index=fm.index)#return fm, ys
        return fm
